...
- Flutter batch flow now selects a saved/local audio file, uploads it through
  the presigned PUT URL, confirms it through
  `POST /meetings/{meeting_id}/upload-confirm`, calls `/start`, persists
  `job_id`, and polls `/jobs/{job_id}` plus `/meetings/{meeting_id}` until
  completion.
...
Purpose:
- Reserve an S3 object key and receive a presigned PUT URL for one saved audio file.
...
- The batch transcript is generated by the backend; Flutter does not upload a
  transcript asset in the current batch contract.

### Confirm Audio Upload

`POST /meetings/{meeting_id}/upload-confirm`

Purpose:
- Ask the backend to verify that the audio object exists in S3 and transition
  the meeting to `uploaded`.
